...
if __name__ == '__main__':
    ...
    # ------------------------ export -----------------------------
    output_script_path = args.save_path
    print("==> Exporting model to TorchScript format at '{}'".format(output_script_path))
    dummy_input = torch.randn(1, 3, cfg['image_size'], cfg['image_size'])  # 입력 크기에 맞게 설정

    # 모델을 TorchScript로 스크립팅
    scripted_model = torch.jit.trace(net, dummy_input)

    # TorchScript 파일 저장
    scripted_model.save(output_script_path)

    print(f'Model has been scripted and saved to {output_script_path}')
